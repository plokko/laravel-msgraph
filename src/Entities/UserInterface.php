...
use plokko\MsGraph\Models\MsLicenseDetails;
...
class UserInterface extends BaseEntity
{
    ...
    function listLicenses($user)
    {
        ...
        return $this->msGraph->graph()
            ->createRequest('get', "/users/$user_id/licenseDetails")
            ->setReturnType(MsLicenseDetails::class)
            ->execute();
    }
     ...
     **/
    function assignLicense($user, $addLicenses = [], $removeLicenses = [])
    {
        $user_id = $user instanceof \Microsoft\Graph\Model\User ? $user->getId() : $user;
